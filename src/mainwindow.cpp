...
void MainWindow::ukui_module_dbus_reg(void)
{
    ...
    QDBusReply<int> reply = interface.call( "module_register", APP_NAME, "support_audio_api", \
                                                "com.kylin.weather", "/com/kylin/weather", "com.kylin.weather", 0);
    ...
    reply = interface.call( "cmd_register",APP_NAME,SHOW_WINDOW, "cmd5");
    if ( reply.isValid() && reply.value() == 0) {
        qDebug( "%d", reply.value());          // prints 4
    } else {
        qDebug() << "fail";
    ...
    reply = interface.call( "mod_register_finish", APP_NAME);
    if ( reply.isValid() && reply.value() == 0) {
        qDebug( "%d", reply.value());          // prints 4
    } else {
        qDebug() << "fail";
...
int MainWindow::ukui_module_exe_cmd_api(int cmd)
{
    ...
    QDBusInterface interface( "org.ukui.audio_api", "/org/ukui/audio_api","org.ukui.audio_api", QDBusConnection::sessionBus());
    QDBusReply<int> reply = interface.call( "cmd_execute_result",APP_NAME, 0, "ok");
    if ( reply.isValid() && reply.value() == 0) {
        qDebug( "%d", reply.value());          // prints 4
    } else {
        qDebug() << "fail";
...
void MainWindow::ukui_module_init()
{
    ukui_module_dbus_reg();
    if(QGSettings::isSchemaInstalled(SETTINGS_AUDIO_API_SCHEMAS))
    {
